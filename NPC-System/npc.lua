...
local function generatePatrolPoint()
    ...
    while attempts < CFG.PATROL_BLOCKED_MAX * 2 do
        ...
        local candidate                      = originPosition +
        ...
        local groundCheck                    = workspace:Raycast(candidate + Vector3.new(0, 10, 0),
...
local function startFlee(myToken)
    ...
    for _, wp in ipairs(path:GetWaypoints()) do
        ...
        while not reached and elapsed < CFG.WAYPOINT_TIMEOUT do
            elapsed += RS.Heartbeat:Wait()
            if state ~= STATE.FLEE or myToken ~= moveToken or isBusy then
...
local function startCombatChase(myToken)
    ...
    task.spawn(function()
        ...
        while state == STATE.COMBAT and myToken == moveToken do
            ...
            local speed = weaponDrawn and
            (myBehaviorRole == "Rusher" and CFG.RUN_SPEED * CFG.RUSHER_SPEED_MULT or CFG.RUN_SPEED * 0.85) or
            CFG.RUN_SPEED
...
local intimidateEvt = game.ReplicatedStorage:FindFirstChild("IntimidateNPC")
    or
    (function()
        local r = Instance.new("RemoteEvent"); r.Name = "IntimidateNPC"; r.Parent = game.ReplicatedStorage; return r
    end)()
        ...
        if canSee(target, CFG.COMBAT_LOS_INTERVAL) then
            ...
            if loseSightTimer >= CFG.LOSE_SIGHT_BUFFER then
                ...
                if lastSeenPos then
                    ...
                    stateDecision.willFight = not stateDecision.willFlee and
                    math.random() < getCharConfig().ALERT_TO_COMBAT
                    enableRotation(false)
                    holsterWeapon()
